...
if (isset($_GET['type'])) {
  $id = $_GET['type'];
  $category = category("books", "$id", $con, );
  ...
  $pagination = getProductDetailsPagenation('books', $start, $ros_per_page, $con);
...
<div class="section-container d-block d-lg-flex gap-5 shop mt-5 pt-5">
  ...
  <div class="shop__products col-12">
    <div class="d-flex justify-content-between align-items-center mb-4">
      ...
      <?php //echo $start ." من اصل  $nr_of_row" ?>
      <?php //echo $ros_per_page . "من اصل " . "$nr_of_row" ?>
      <form action="">
        <div class="filter__search position-relative">
    ...
    <?php else: ?>
      ...
      <div class="products__pagination mb-5 d-flex justify-content-center gap-2">
        <?php if (isset($_GET['page-nr']) && $_GET['page-nr'] > 1) { ?>
          ...
          <?php } else { ?>
            <a class="page-link rounded-0 shadow-sm border-top-0 border-left-0 text-dark" href="#"> <i
                class="fa-solid fa-arrow-right-long"></i></a> <?php
        }
        ?>
        ...
        <?php for ($counter = 1; $counter <= $pages; $counter++) { ?>
          <span class="pagination__btn rounded-1 active">
            <a class="pagination__btn rounded-1"
              href="<?php echo $config['base_url']; ?>index.php?page=shop&page-nr=<?php echo $counter ?>"><?php echo $counter ?></a>
          </span>
        <?php } ?>
        ...
        <span class="pagination__btn rounded-1 active">
          <?php if (!isset($_GET['page-nr'])) {
            ?>
            <a href="<?php echo $config['base_url']; ?>index.php?page=shop&page-nr=2"
              class="page-link rounded-0 shadow-sm border-top-0 border-left-0 text-dark">Next</a>
            <?php
